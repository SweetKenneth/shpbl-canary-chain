 ...
 * stdin/stdout only — no socket, no fetch, no file write, no process spawn.
 */
import { CanaryChain } from "./canary-chain.js";
import { CanaryError } from "./canonical.js";
...
export async function handle(message: any): Promise<void> {
  ...
  try {
    switch (method) {
      case "initialize":
        return respond(id, {
          ...
          serverInfo: { name: "shpbl-canary-chain", version: "0.1.0" },
          capabilities: { tools: {} },
        });
      case "tools/list":
        return respond(id, { tools: TOOLS });
      case "tools/call": {
        const result = await callTool(chain, params?.name, params?.arguments ?? {});
    ...
    if (err instanceof CanaryError) return fail(id, -32602, err.code, err.toJSON());
